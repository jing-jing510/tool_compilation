...
def request_with_retry(messages, max_tokens=3000, max_requests=90, cooldown_seconds=60):
    while True:
        try:
...
def rewriteText(text):
    messages = [
        {"role": "system", "content": "我想让你扮演优秀写手的角色，你会把我给你的每一句话进行更改，但是不改变原来的意思，同时还做到幽默有趣,更能够吸引人看下去"},
        {"role": "user", "content": f",文本内容为:{text}"},
    ]
    return request_with_retry(messages)
